...
fn main() {
    ...
    let result: i32 = content.lines().map(get_valid_game_id).sum();
    let power: i32 = content.lines().map(get_power_values).sum();

    println!("Sum of all game ID is: {result}");
    println!("Sum of the power is: {power}");
}

fn get_valid_game_id(line_value: &str) -> i32 {
    let game_info = line_value.split(':').collect::<Vec<&str>>();
        ...
        for round in game {
            if let [amount, color] = round.split(' ').collect::<Vec<&str>>()[0..] {
                ...
                while index < minimum_game_value.len() {
                    let should_update_min_value = minimum_game_value[index].0 == color
                        && minimum_game_value[index].1 < amount;

                    if should_update_min_value {
                        minimum_game_value[index].1 = amount;
                    }
